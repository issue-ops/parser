...
async function run() {
    ...
    if (fs_1.default.existsSync(`${workspace.replace(/\/+$/, '')}/.github/ISSUE_TEMPLATE/${template}`) === false) {
    ...
    // Parse the issue
    const parsedIssue = await (0, parse_1.parseIssue)(body, parsedTemplate);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.parseTemplate = exports.parseIssue = void 0;
const core = __importStar(__nccwpck_require__(2186));
...
async function parseIssue(body, template) {
    ...
    for (const match of matches) {
        ...
        value = (0, format_1.formatValue)(value, template[key]);
        if (value === null) {
            core.warning(`Skipping invalid value for key: ${key}`);
            continue;
        }
        core.info(`Formatted Key: ${key}`);
        core.info(`Formatted Value: ${typeof value === 'string' ? value : JSON.stringify(value)}`);
...
async function parseTemplate(template) {
    ...
    for (const item of template.body) {
        ...
        const formattedKey = (0, format_1.formatKey)(item.attributes.label);
